...
def direction_compatible(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    for speed in (upstream.get("avg_speed"), downstream.get("avg_speed")):
        numeric_speed = _finite_nonnegative_number(speed)
        if numeric_speed is not None and numeric_speed < _MIN_MOVING_SPEED_PX_PER_SEC:
            return False
...
def speed_compatible(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    left_speed = _finite_nonnegative_number(left.get("avg_speed"))
    right_speed = _finite_nonnegative_number(right.get("avg_speed"))
