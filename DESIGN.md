...
The rewrite prompt permits the complete parser-supported operator set,
including `Detect`, `Join`, `Window`, and `Coalesce`, and requires explicit
`mmds` operator and `udfs.*` imports. The LLM optimizer is currently a
controlled interface, not a production optimizer. Static-client tests validate
the rewrite contract, parsing, input preservation, and semantic equivalence on
controlled fixtures; they do not establish that a live model will discover a
...
- `Join` validation, keyed and predicate execution, greedy one-to-one scoring,
  shared self-join execution, and cross-camera trajectory normalization
- parse/render equivalence for `Join`, extended `Detect`, `Window`, and `Coalesce`
...
- more operators beyond the current unary and binary set
